...
def add_movie(movie_id: int):
    ...
    movie = Movie(
        ...
        director=', '.join([crew['name'] for crew in tmdb.Movies(movie_id).credits()['crew'] if crew['job'] == 'Director']),
...
def search_movie_by_title(title: str) -> list[MovieSearchResult]:
    ...
    for result in data.results:
        list_films.append(MovieSearchResult(title=result['title'], poster_path=result['poster_path'], release_date=result['release_date'], id=result['id']))
    return list_films
